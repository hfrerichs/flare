...
module modtopo_cdn
 ...
 1003 format(12x,   6x,a6,3x'(',i0,' x ',i0,' x ',i0,')')
  ...
  do iblock=0,blocks-1
     ...
     call make_flux_surfaces_PFR(M_PFR1, nr(3), npL(1), npR(1), rpath(3), Zone(iz0+3)%Sr, Zone(iz0+3)%Sp)
     call make_flux_surfaces_PFR(M_PFR2, nr(4), npR(2), npL(2), rpath(4), Zone(iz0+4)%Sr, Zone(iz0+4)%Sp)
